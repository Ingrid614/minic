...
public class Block {
	 ...
	 */	
	// nous avons modifié le allocateMemory afin qu'il retourne un int pour pouvoir récupérer la taille d'un bloc
	public int allocateMemory(Register _register, int _offset) {
		int offset_updated = _offset;
		for(Instruction i : this.instructions){
			offset_updated += i.allocateMemory(_register, offset_updated);
		}
		return (offset_updated - _offset);
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		for(Instruction instr : this.instructions){
			fragment.append(instr.getCode(_factory));
		}
		return fragment;
	}
