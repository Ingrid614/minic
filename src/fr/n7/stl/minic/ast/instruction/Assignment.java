...
public class Assignment implements Instruction, Expression {
	...
	public boolean checkType() {
		Type assignedType = this.assignable.getType();
		Type valueType = this.value.getType();
		if(assignedType == null || valueType == null){
			Logger.error("Undefined type for one or both expressions");
			return false;
		}
		if(!assignedType.compatibleWith(valueType)){
			Logger.error("Type mismatch : cannot convert "+assignedType+" to "+valueType);
			return false;
		}
		return true;
	}
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		fragment.add(
			_factory.createStoreI(
				this.assignable.getType().length()
			)
		);
